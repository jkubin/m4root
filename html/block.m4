...
__HEADER(⟦Josef Kubin⟧, ⟦2019/12/29⟧)
...
pushdef(⟦CLASS_3_ROOT_CMD⟧,	⟦ class="ADD_CLASS(⟦cmd⟧) ADD_CLASS(⟦root⟧)⟧defn(⟦CLASS_3_SUFFIX⟧))
...
# β
#pushdef(⟦PROCESS_RAW_CODE_TO_HTML_ENTITIES⟧, ⟦patsubst(patsubst(patsubst(patsubst(patsubst(define(⟦#⟧, ⟦⟦⟦⟦⟦⟦$$#⟧⟧⟧⟧⟧⟧)indir(⟦#⟧, $@), ⟦\s*\s*⟧), ⟦&⟧, ⟦&amp;⟧), ⟦<⟧, ⟦&lt;⟧), ⟦>⟧, ⟦&gt;⟧), ⟦\<dnl\>\|#⟧, ⟦⟦\&⟧⟧)⟧)
...
pushdef(⟦SET_ANCHOR⟧, ⟦
	...
	ifdef(defn(⟦FILE_PREFIX⟧).uniq.defn(⟦SELITM⟧), ⟦⟧, ⟦
		...
		ROOT_WARNING(⟦unknown $0(⟧PAYR(defn(⟦SELITM⟧))⟦) found; run ‘make -B refs …’ to regenerate⟧)
	⟧)
