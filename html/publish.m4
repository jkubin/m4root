...
# the header helps to maintain consistency: source_file.mc, git_revision, time_stamp
divert(BEGIN_OF_ARTICLE)dnl
<!-- generated by M4 __SOURCE(LB()defn([SOURCE]), ARG1(esyscmd([git log -1 --format='[0x%h],' ]defn([SOURCE]))), ARG1(esyscmd([date '+[%Y%m%d-%R:%S],']))]) -->
...
# copy the article to the clipboard using the keyboard shortcut (xvim): "+yG
