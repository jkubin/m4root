...
TARGET_FOLDER/%.html: html_$2.m4f $(JAVASCRIPT) %.m4 $1 nav.m4
	m4 -DSOURCE='$1' -DOUTPUT_FILE='$[@]' -DARTICLE_PATH='TARGET_FOLDER' $(FLAGS) -R $(filter-out $(JAVASCRIPT), $^) | sed -f html/esc_to_ent.sed > $[@]
	tidy -qe $[@]

TARGET_FOLDER/publish.txt: html_$2.m4f $(JAVASCRIPT) publish.m4 $1 nav.m4
	m4 -DSOURCE='$1' -DARTICLE_PATH='TARGET_FOLDER' $(FLAGS) -R $(filter-out $(JAVASCRIPT), $^) | sed -f html/publish.sed -f html/esc_to_ent.sed > $[@]
