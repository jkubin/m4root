...
Vícejazyčný seriál QUOTE([BO([SERIES_NAME])]) je generován M4 skripty[]REF([SERIES_NAME], [vícejazyčná šablona s[]NB()příklady pro AD([www.root.cz], [Root.cz – informace nejen ze světa Linuxu], [https://www.root.cz])], [http://github.com/jkubin/m4root]),
které (možná) usnadní psaní článků a[]NB()seriálů pro AD([www.root.cz], [Root.cz – informace nejen ze světa Linuxu], [https://www.root.cz/]) také jiným autorům.
],
[dnl english: _next_language_
Multilingual series QUOTE([BO([SERIES_NAME])]) are generated by M4 scripts[]REF([SERIES_NAME], [multilingual template with examples for AD([www.root.cz], [Root.cz – information not only from the Linux world], [https://www.root.cz])], [http://github.com/jkubin/m4root]),
which will make it easier (maybe) for other authors to write articles on AD([www.root.cz], [Root.cz – information not only from the Linux world], [https://www.root.cz/]).
]])
])
